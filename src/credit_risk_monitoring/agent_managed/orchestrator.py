...
def _as_dict(obj: Any) -> dict[str, Any]:
    ...
    if callable(dump):
        try:
            return dict(dump())
        except Exception:  # noqa: BLE001 — fall through to attribute scan
            pass
...
class ManagedOrchestrator:
    ...
    def investigate(self, fixture: Fixture, *, trace_dir: Path) -> ManagedRunResult:
        ...
        try:
            ...
            logger.exception("arm-b investigation %s crashed", fixture.id)
            if not final_answer:
        ...
        if session_id:
            try:
                ...
                logger.debug("arm-b: session usage retrieve failed for %s", fixture.id)
